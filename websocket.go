...
type WebSocketDownstreamMessage struct {
	*WebSocketMessage
	// Sn      string          `json:"sn"` // 23.08.29 comment out this for now, since kc change this field to int
	Topic   string          `json:"topic"`
...
func (wc *WebSocketClient) read() {
	...
	for {
		...
		case <-wc.done:
			return
		default:
			_, d, err := wc.conn.ReadMessage()
			if err != nil {
				wc.errors <- fmt.Errorf("read message failed, %s", err.Error())
				return
			}

			m := &WebSocketDownstreamMessage{}
			if err := json.Unmarshal(d, m); err != nil {
				wc.errors <- fmt.Errorf("unmarshal message failed, data: %s, err: %w", string(d), err)
				return
			}
